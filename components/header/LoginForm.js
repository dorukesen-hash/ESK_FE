...
const LoginForm = ({setUserMenuOpen}) => {
	...
	const onSubmit = async (data) => {
		...
		try {
			...
			if (response.status === 200) {
				...
				if (response.data && response.data?.data?.isAdmin === "admin") {
					router.push("/admin");
				} else if (response.data && !response.data?.data?.isAdmin) {
					router.push("/auth/my-account");
				}
				successNote(`Logged Successfully`)
			}
		} catch (err) {
	...
	return (
		<div className="w-full m-0">
			<form onSubmit={handleSubmit(onSubmit)} noValidate>
				...
				<div className="mb-4">
					<input
						...
						{...register("email")}
						placeholder="Email"
						className={`w-full h-[50px] text-[18px] px-3 py-2 border ${errors.email ? "border-red-500" : "border-gray-300"} rounded-[6px]`}
					/>
				...
				<div className="mb-[16px]">
					<input
						...
						{...register("password")}
						className={`w-full h-[50px] px-3 py-2 text-[18px] border ${errors.password ? "border-red-500" : "border-gray-300"} rounded`}
					/>
					{errors.password && <p className="text-red-500 text-sm mt-1">{errors.password.message}</p>}
				</div>
				<div className="flex items-center mb-[16px] relative">
					<label htmlFor="remember-header" className="flex items-center relative cursor-pointer select-none">
						<input
							id="remember-header"
							type="checkbox"
							checked={rememberMe}
			...
			<p className="text-center my-[20px]">
				<Link href={"/auth/forgot-password"} className="w-full text-[16px] text-custom-blue font-semibold">
					Forgot Password
				</Link>
			</p>

			<svg width="432" height="22" viewBox="0 0 432 22" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M0 11H196" stroke="#BDC2C7" strokeMiterlimit="10"/>
				<path d="M236 11H432" stroke="#BDC2C7" strokeMiterlimit="10"/>
				<path
